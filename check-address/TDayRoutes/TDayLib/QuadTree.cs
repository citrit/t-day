...
namespace Auios.QuadTree
{
    ...
    public class QuadTree<T>
    {
        ...
        public void GetDeliveries(Dictionary<int, T[]> deliveries)
        {
            if (!_hasChildren)
            {
                deliveries.Add(this.GetHashCode(), FindObjects(Area));
            }
            else
            {
                quad_TL.GetDeliveries(deliveries);
                quad_TR.GetDeliveries(deliveries);
                quad_BL.GetDeliveries(deliveries);
                quad_BR.GetDeliveries(deliveries);
            }
        }
